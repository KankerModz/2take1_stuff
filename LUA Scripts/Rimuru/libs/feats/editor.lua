...
local worldObjectsFeat = func.add_feature("World Objects", "action_value_str", spawnOpt.id, function(feat)
...
local filteredObjs = {}
func.add_feature("Filter Objects", "action", spawnOpt.id, function(feat)
...
local filteredFeat = func.add_feature("Filtered Objects", "parent", spawnOpt.id)
    ...
    for i=1, #spawnedEditor.objects do
        ...
        if obj ~= nil then
            if not spawnedObjFeats[obj] then
                spawnedObjFeats[obj] = func.add_feature(tostring(gameplay.get_hash_key(obj)), "parent", spawnedObjs.id)
    ...
    for i=1, #spawnedEditor.peds do
        ...
        if ped ~= nil then
            if not spawnedPedFeats[ped] then
                spawnedPedFeats[ped] = func.add_feature(tostring(getPedFromHash(gameplay.get_hash_key(ped))), "parent", spawnedPeds.id)
